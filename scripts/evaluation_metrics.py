...
def perplexity_score(predictions, model_id, peft_model_id=None, is_llama=False, use_auth_token=None):
	perplexity = load("perplexity", module_type="metric")
	results = perplexity.compute(predictions=predictions, model_id=model_id, peft_model_id=peft_model_id, is_llama=is_llama, use_auth_token=use_auth_token)
	return results['mean_perplexity']

def main():
	model_and_tokenizer_path = "NorGLM/NorGPT-369M"
	
	fnames = [('storytelling_369M.csv', 'NorGLM/NorGPT-369M')
			]
	...
	for item in fnames:
		...
		hypo_list = df['generated_text'].to_list()

		bleu1,rouge1,rouge2,rougel,dist_1,dist_2,dist_3,dist_4,entr,usr,mauve = eva_pairs(references, hypo_list)
		...
		#ent_ratio1, neu_ratio1, con_ratio1 = entailment_score(articles, references, hypo_list)
