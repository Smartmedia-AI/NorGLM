...
def main():
	...
	fnames = [#('storytelling_369M.csv', 'NorGLM-369M'),
			...
			#('summarization_23B.csv', 'NorGPT_23B'),
			#('summarization_Llama3B.csv', 'NorLlama_3B_HF'),
			# ('summarization_3B_continue.csv', 'NorGPT-3B-continue'),
			# ('summarization_369M.csv', 'NorGLM-369M'),
			# ('summarization_3B.csv', 'NorGLM-3B'),
			# ('summarization_NbAiLab6B.csv', 'NbAiLab/nb-gpt-j-6B'),
			#('summarization_rlhp.csv', 'NorGLM-3B-rlhf'),
			#('summarization_23B_new.csv', 'NorGPT_23B'),
			('summarization_GPT3.5.csv', 'GPT3.5')
			]
			
	fout = open("results_sum.txt", "a")
...
if __name__ == "__main__":
    ...
    '''
    fpath = './data/'
    #fnames = ['actor_eva_stage3_instruct_clean.csv', 'aema_eva_stage3_instruct_clean.csv', 'finetune_eva_stage3_instruct_clean.csv']
    fname = 'pretrain_eva_stage3_instruct_clean.csv'
    ...
    print(len(df))
    references = df['positive_sample'].to_list()
    texts = df['article'].to_list()
    generated_texts = df['pretrain_generated1'].to_list()

    entailment_score(texts, references, generated_texts)
    '''
